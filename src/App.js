...
function App() {
  ...
  const Ques = orderedQues.map((ordQues) => (
    <QuesCard key={ordQues.Id} {...ordQues} selectOption={selectOption} />
  ));
  ...
  function loadQues() {
    setOrderedQues((prevState) => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        tempOrderedQues.Option = [
          { value: data[i].incorrect_answers[0], isHeld: false, result: 0 },
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (tempOrderedQues.Id === ID) {
          for (let j = 0; j < tempOrderedQues.Option.length; j++) {
            ...
            else tempOrderedQues.Option[j].isHeld = false;
          }
        }
      ...
      for (let i = 0; i < 5; i++) {
        ...
        for (let j = 0; j < tempOrderedQues.Option.length; j++) {
          if (tempOrderedQues.Option[j].value === tempOrderedQues.Answer) {
            ...
            tempOrderedQues.Option[j].isHeld &&
            tempOrderedQues.Option[j].value !== tempOrderedQues.Answer
          ) {
